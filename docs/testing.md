...
| `tests/test_visibility.py` | `build_web.py`'s `_visible_for_homepage()` and `impact.py`'s `_visible_for_impact()` — both independently exclude only `personal_only`, and are asserted to agree with each other on every visibility value. Also Contacts Phase 4 — proves `contacts.py`'s `is_public_visible()` is deliberately STRICTER than the homepage bar (diverges on `communardo_internal`/`communardo_management`), and `build_web.py`'s `compute_public_contacts()` excludes the fixture's default-visibility contact but includes one explicitly set to an external-sharing tier, with PII (email) stripped |
...
| `tests/test_html_checks.py` | `scripts/check_html.py` against the real `dashboard.html`/`web/index_template.html` — no duplicate element IDs, every `VIEWS` entry has a matching `#view*`/`#tab*` pair. Covers both files' Contacts tabs (Phase 3 dashboard, Phase 4 public site) — each `VIEWS` array includes `contacts` with matching `#viewContacts`/`#tabContacts` |
...
| `tests/test_contacts.py` | `scripts/contacts.py` (Contacts Phase 1 / R3-T01) — name normalisation/similarity, `find_candidate_matches()`/`match_contact()` auto-match vs. needs-review vs. new-provisional decisions, `find-or-create`/`set-canonical-name`/`merge` CLI workflow, evidence supersession (higher-or-equal confidence updates the profile, lower confidence does not), `compute_profile_summary()`'s confirmed/probable/subjective separation and missing-information flagging. Also Phase 2 (batch document ingestion) — `record_evidence_row()`/`resolve_or_create_for_ingest()` as pure functions, `cmd_ingest`'s matched/new/needs-review paths, `--dry-run` writes nothing, malformed-payload all-or-nothing rejection (intentional failure), `possible_duplicate` flagging on a needs-review ingest never silently merging, and `compute_profile_summary()` surfacing an unresolved `possible_duplicate` near the top of the rendered text. Also Phase 3 — `possible_duplicate_flags()`'s candidate_id parsing (used by the dashboard's Contacts tab), excluding superseded rows and other contacts' rows. Also Phase 4 — `is_public_visible()` excludes every internal-default tier and includes only the four explicit external-sharing tiers; `public_contact_view()` strips sensitive/subjective evidence, superseded/system rows, and never includes email/phone or any internal-only field |
