...
| `tests/test_html_checks.py` | `scripts/check_html.py` against the real `dashboard.html`/`web/index_template.html` — no duplicate element IDs, every `VIEWS` entry has a matching `#view*`/`#tab*` pair. Covers the Contacts tab (Phase 3) too — `VIEWS` includes `contacts` with matching `#viewContacts`/`#tabContacts` |
